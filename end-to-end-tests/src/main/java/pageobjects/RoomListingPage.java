...
public class RoomListingPage extends BasePage {
    ...
    @FindBy(how = How.CSS, using = "div[data-type~=\"room\"]>div>span[class='fa fa-remove roomDelete']")
    private List<WebElement> lstRoomDelete;
    ...
    public void roomsToDelete() throws InterruptedException {
        int sizeNo = lstRoomDelete.size();
      for(int i=0;i<sizeNo;i++){
          lstRoomDelete.get(0).click();
          Thread.sleep(2000);
      }
    }
    ...
    public int generateRandomNumber(int number){
        ...
        int int_random = rand.nextInt(number);
        return int_random;
    }
